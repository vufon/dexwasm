...
var (
	...
	bisonwAppDir      = dcrutil.AppDataDir("dexc", false)
	defaultConfigPath = filepath.Join(appDir, defaultConfigFilename)
)

// config defines the configuration options for bwctl.
type config struct {
...
func configure() (*config, []string, bool, error) {
	...
	if cfg.RPCCert == "" {
		...
		if !dex.FileExists(cfg.RPCCert) { // Then in ~/.dexc
			cfg.RPCCert = dex.CleanAndExpandPath(filepath.Join(bisonwAppDir, defaultRPCCertFile))
		}
	} else {
